      ...
      <div className="container">
        <div className="position-relative">
          <div className="title-one mb-35 xl-mb-20 md-mb-10 wow fadeInUp">
            {style ? (
              <h3>
                Latest{" "}
                <span>
                  News <Image src={titleShape} alt="" className="lazy-img" />
                </span>
              </h3>
            ) : (
            ...
            {blog_data
              ...
              .map((item) => (
                <div key={item.id} className="col-md-6">
                  ...
                  >
                    <figure
                      className={`post-img position-relative m0 ${item.class_name} ${style ? "rounded-5" : ""}`}
                    >
                      <Link
                        href="/blog_details"
                        className={`stretched-link date tran3s ${style ? "rounded-5" : ""}`}
                      >
                        {item.date}
                      </Link>
                    </figure>
                    <div className="post-data">
                      <div className="post-info">
                        <Link href="/blog_details">{item.info_name}</Link>{" "}
                        {item.info_time} min
                      </div>
                      <div className="d-flex justify-content-between align-items-sm-center flex-wrap">
                        ...
                        <Link
                          href="/blog_details"
                          className={`read-btn d-flex align-items-center justify-content-center tran3s ${style ? "rounded-circle" : ""}`}
                        >
          ...
          <div className="section-btn text-center md-mt-60">
            <Link href="/blog" className="btn-eight">
