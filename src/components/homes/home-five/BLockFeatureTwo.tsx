...
const BLockFeatureTwo = ({ style }: any) => {
  return (
    <div
      className={`block-feature-twelve ${style ? "mt-150 xl-mt-100" : "mt-170 xl-mt-120"}`}
    >
      <div className="container container-large">
        <div className="position-relative">
          ...
          <div className="wrapper flex-wrap d-flex justify-content-center justify-content-md-between align-items-center">
            {feature_data
              .filter((items) => items.page === "home_5_feature_1")
              .map((item) => (
                <div
                  key={item.id}
                  className="location-card-three text-center wow fadeInUp"
                  data-wow-delay={item.data_delay_time}
                >
                  <div
                    className={`image-bg position-relative z-1 rounded-circle overflow-hidden mb-30 ${item.item_bg}`}
                  >
                    <Link href="/listing" className="stretched-link"></Link>
                  </div>
                  <Link href="/listing" className="title">
                    <h5>{item.title}</h5>
                  </Link>
                  <p>{item.desc}</p>
                </div>
